...
```{r changercp, fig.height = 20/2.54/2, fig.cap="Changement relatif moyen (ligne) et incertitude (plage colorée) du module annuel de la Seine à Poses en fonction du scénario RCP considéré (référence 1990). L'incertitude fournie est calculée à partir de la variance avec un intervale de confiance 0.9 (quantiles 5/95%)."}
...
```{r mapquanthoriz, fig.cap="Carte du changement relatif du module annuel de débit pour la France métropolitaine en RCP 8.5, par rapport à la référence 1990, à 3 horizons temporels: 2030, 2050 et 2085 (en lignes). En colonnes sont donnés la moyenne d'ensemble et l'intervalle de confiance 5/95% obtenus avec la méthode QUALYSPO. Cette intervalle de confiance prend en compte la dispersion entre les GCMS, les RCM et la variabilité résiduelle. Points à l'exutoire."}
...
1. La figure précédente montrait l'incertitude sur le signal de réponse au changement climatique, sans prise en compte de la variabilité interne du système climatique. Pour référence l'incertitude (écart-type) de cette variabilité est donné ici, afin de mettre en perspectives signaux de changements et variabilité interne.

2. Cette incertitude est comprise entre 30 et 80% environ, avec un maximum sur le pourtour Méditerranéen.



```{r mapgcmeff, fig.height = 20/2.54/1.7, fig.cap="Carte des effets principaux des GCMs pour le module annuel de débit sur la France métropolitaine en 2050, par rapport à la référence 1990. Points à l'exutoire."}
...
On représente cette fois les effets principaux de chaque RCM. Le résultat marquant ici est une illustration de l'utilité d'étudier les effets principaux. On peut voir qu'avec notre sous-ensemble arbitraire d'EXPLORE 2 le RCM WRF381P a un effet principal particulièrement marqué sur le bassin Parisien. Il produit donc des modules annuels de débit environ 40% plus élevé que la moyenne d'ensemble. On a donc pu mettre en évidence (pour cet exemple arbitraire) un maillon de la chaîne de modélisation particulièrement intéressant à étudier en détail si l'intérêt de l'utilisateur se porte sur les fortes augmentations ou à retirer si ce dernier se porte sur des scénarios plus médians.

```{r mapgcmch, fig.height = 20/2.54/1.7, fig.cap="Carte des changements relatifs pour le module annuel de débit en fonction du GCM sur la France métropolitaine en 2050, par rapport à la référence 1990. Les valeurs sont les moyennes d'ensemble complet pour un GCM donné. Points à l'exutoire."}
...
```{r maprcmch, fig.cap="Carte des changements relatifs pour le module annuel de débit en fonction du RCM sur la France métropolitaine en 2050, par rapport à la référence 1990. Les valeurs sont les moyennes d'ensemble complet pour un RCM donné. Points à l'exutoire."}
