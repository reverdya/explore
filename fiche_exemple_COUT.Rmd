...
2. Le graphique est à interpréter avec précaution. L’incertitude liée à la variabilité interne n’a jamais tendance à diminuer avec le temps (ce qu’une mauvaise interprétation de la figure pourrait suggérer). Par hypothèse de la méthode d’estimation QUALYPSO, elle est en effet considérée invariante sur toute la période de temps. Sa contribution relative à l’incertitude totale diminue à l’inverse (ce que l’on voit sur la figure); ce qui est logique puisque toutes les autres sources d’incertitude augmentent avec le temps, conduisant à une augmentation de l’incertitude totale sur la période. 
...
1. Le changement relatif moyen du débit annuel estimé pour l’ensemble EXPLORE 2 est à la baisse, de l’ordre de +30% en fin de siècle.
...
```{r changercp, fig.height = 20/2.54/2, fig.cap="Changement relatif moyen (ligne) et incertitude (plage colorée) du module annuel de la Seine à Poses en fonction du scénario RCP considéré (référence 1990). L'incertitude fournie est calculée à partir de la variance avec un intervalle de confiance 0.9 (quantiles 5/95%)."}
...
1. On observe par exemple ici que si les RCP 2.6 et 4.5 sont plutôt en accord sur une augmentation du module annuel d'environ 20% en 2085, en RCP 8.5 cette augmentation est de l'ordre de 30%. Néanmoins l’incertitude liée à la dispersion des chaînes de modélisation est importante et les projections des 3 RCP se recoupent donc fortement, avec une incertitude liée à la dispersion des chaînes de modélisation en 2085 comprise entre -30 et +55% pour les 2 premiers et entre -20 et +75% pour le RCP 8.5.

2. On note que dans notre exemple l'incertitude entre les chaînes de modélisation est bien plus importante que la dispersion entre les scénarios RCP.
